...
const DuaCard = ({dua}) => {
    console.log(dua);
    return (
        <div className="flex flex-col gap-4 w-full bg-white p-4 rounded-xl mb-6" >
            <div className="flex gap-2  items-center main-clr font-semibold">
                <Image src={AllahLogo} alt="Allah name" width={40} height={40} className="rounded-full md:w-fit md:aspect-square md:object-cover w-10"></Image>
                <p> <span>{dua?.dua_id}. </span> {dua?.dua_name_en}</p>
            </div>
            ...
            <FaPlayCircle className="main-clr"/>
                </div>
